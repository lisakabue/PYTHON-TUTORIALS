...
def count_sentences(text):
    sentence_endings = '.!?'
    return sum(text.count(char) for char in sentence_endings)
...
def main():
    ...
    print(f"The word '{target_word}' appears {count_word(news_article, target_word)} times.\n")
    
    # Most common word
    common_word, freq = most_common_word(news_article)
    print(f"The most common word is '{common_word}', which appears {freq} times.\n")
    
    # Average word length
    avg_len = average_word_length(news_article)
    print(f"The average word length is {avg_len:.2f} characters.\n")
    
    # Paragraph count
    print(f"The article contains {count_paragraphs(news_article)} paragraphs.\n")
    
    # Sentence count
    print(f"The article contains {count_sentences(news_article)} sentences.\n")
